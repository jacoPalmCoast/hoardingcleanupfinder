...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (action === 'save') {
    ...
    const state = clean(form.get('state'), 2).toUpperCase() || l.state;
    ...
    await env.DB.prepare(
      ...
      .bind(id, clean(form.get('name'), 120) || l.name, phone || null, digits(phone) || null, safeUrl(clean(form.get('website'), 200)), clean(form.get('email'), 254) || null, clean(form.get('address'), 200) || null, city, slugify(city), state, clean(form.get('zip'), 5) || null, JSON.stringify(services.length ? services : ['hoarding-cleanup']), clean(form.get('description'), 600) || null, form.get('is_verified') ? 1 : 0, status)
      .run();
